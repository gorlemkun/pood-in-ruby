...
これにより、車輪の直径をひとつだけ計算するdiameterメソッドが生まれ、再利用しやすくなった。

また、以前のGearクラスのgear_inchesメソッドを思い出してみる。
...
こちらも2つの責務を持っていないだろうか？

よく見ると、このメソッドは

- ratio * タイヤの直径からなるgear_inches を計算する
- タイヤの直径を計算する
...
単一責任のメソッドがもたらす恩恵は次のようになる。

- 隠蔽されていたクラスの性質を明らかにする
  - まさに今起こったように。
- コメントする必要がなくなる
  - コメントは実行できないため、更新されず朽ち果ててゆくことが多い。
- 再利用を促進する
